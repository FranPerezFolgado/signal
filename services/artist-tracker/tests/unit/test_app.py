...
def _run_one_cycle(artists, tracks_by_artist=None, spotify_errors=None):
    """
    Drive one full cycle through run(), then stop immediately.
    ...
    artist_repo.get_eligible.return_value = artists

    spotify = MagicMock()

    def top_tracks_side_effect(artist_uri):
        name = next((a["name"] for a in artists if a.get("external_ids", {}) and a["external_ids"].get("spotify") == artist_uri), None)
    ...
    call_count = [0]
    original_get_eligible = artist_repo.get_eligible
...
def test_eligible_artists_fetched_and_tracks_emitted():
    ...
    # Verify message schema for the first produced message
    call_args = producer.produce.call_args_list[0]
    _, msg, _ = call_args[0][0], call_args[0][1], call_args[1]
...
def test_spotify_error_skips_mark_explored_continues_next_artist():
    ...
    # good artist was still processed
    assert producer.produce.call_count == 1
    # mark_explored called only for good artist

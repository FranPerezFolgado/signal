...
def _make_client(max_retries: int = 3) -> SpotifyClient:
    with patch.object(SpotifyClient, "_refresh_access_token"):
...
class TestRefreshAccessToken:
    ...
    def test_failure_raises_auth_error(self) -> None:
        ...
        assert "secret" not in str(exc_info.value)  # credentials not leaked
...
class TestGet:
    ...
    def test_double_401_raises_auth_error(self) -> None:
        client = _make_client()
        with patch("signal_normalizer.spotify_client.requests.get") as mock_get:
